...
function InfoRemove() {
    ...
    useEffect(function () {
        (async function () {
            await infoservice.getListRemove(8, page).then(function (result) {
    ...
    function infoRestore(id) {
        infoservice.restore(id).then(function (result) {
    ...
    function infoDelete(id){
        infoservice.remove(id).then(function(result){
            console.log(result.data.message);
            setStatusDelete(result.data.id);
    ...
    return ( 
        <div className="card">
             <div className="card-header">
                <div className="row">
                    <div className="col-6">
                    <Link to="/admin/info"><strong className="text-primary">THÔNG TIN CỬA HÀNG</strong></Link>
                        </div>
                    ...
                    {infos.map(function(info,index){
                        return( <tr key={index}>
                            ...
                            <td>
                               ...
                               <FaEye/> </Link>
                               <button onClick={() => infoRestore(info.id)} className="btn btn-sm btn-danger me-1">
